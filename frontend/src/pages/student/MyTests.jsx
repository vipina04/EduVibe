...
const MyTests = () => {
  ...
  const handleSubjectClick = async (subject) => {
    ...
    try {
      // GET /students/subjects/:id/chapters/ → returns { subject, class, chapters[] with tests[] }
  ...
  // ✅ Just navigate — TakeTest.jsx handles the API call itself
...
// Chapter Card — collapsible, shows tests inside
          ...
          <div className="text-left">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {chapter.is_completed && (
                <span className="flex items-center gap-1 text-xs font-medium text-green-600 bg-green-50 dark:bg-green-900/20 px-2 py-0.5 rounded-full">
            ...
            <div className="flex items-center gap-3 mt-1 text-sm text-gray-500 dark:text-gray-400">
              ...
              {attemptedTests.length > 0 && (
                <><span>•</span><span className="text-green-600">{attemptedTests.length} done</span></>
              )}
              {pendingTests.length > 0 && (
                <><span>•</span><span className="text-orange-500">{pendingTests.length} pending</span></>
              )}
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-1 text-sm text-gray-500 dark:text-gray-400">
                  ...
                  <span>•</span>
                  <span className="uppercase text-xs font-medium text-blue-600">{test.type}</span>
                  {test.attempted && test.percentage !== null && (
                    <><span>•</span>
                    <span className="text-green-600 font-medium">Score: {test.percentage}%</span></>
                  )}
                </div>
              </div>
...
//     setLoadingChapters(true);
//     try {
//       // ✅ /students/subjects/:id/chapters/ returns chapters WITH full tests array
//       const response = await studentAPI.getChapters(subject.id);
//       console.log('Chapters+Tests:', response.data);
//       setChaptersData(response.data);
//     } catch (error) {
//       console.error('Failed to load chapters:', error);
//       toast.error('Failed to load chapters');
...
// // Chapter Card — collapsible, shows tests inside
// const ChapterCard = ({ chapter, index, onStartTest, startingTest }) => {
//   const [expanded, setExpanded] = useState(false);

//   // ✅ tests array comes from StudentSubjectChaptersView
//   const tests = chapter.tests || [];
//   const totalTests = tests.length || chapter.test_count || 0;
//   const pendingTests = tests.filter(t => !t.attempted);
//   const attemptedTests = tests.filter(t => t.attempted);
...
//                     className="flex items-center gap-2 px-4 py-1.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors disabled:opacity-60"
//                   >
...
// //   const [startingTest, setStartingTest] = useState(null); // testId being started
...
// //       setLoadingSubjects(true);
// //       const response = await studentAPI.getSubjects();
// //       // getSubjects returns { class, subjects, total_subjects }
// //       const data = response.data;
// //       const list = Array.isArray(data) ? data : (data.subjects || []);
// //       setSubjects(list);
// //     } catch (error) {
...
// //   // ── Load chapters+tests for a subject ────────────────────
// //   const handleSubjectClick = async (subject) => {
// //     setSelectedSubject(subject);
// //     setStep('chapters');
// //     setChaptersData(null);
// //     setLoadingChapters(true);
// //     try {
// //       // GET /students/subjects/:subjectId/ → returns { subject, class, chapters }
// //       // const response = await studentAPI.getSubjectDetails(subject.id);
// //     //   const response = await studentAPI.getChapters(subject.id);

// //     //   // setChaptersData(response.data);
// //     //   // getChapters returns { subject, class, chapters, total_chapters }
// //     // setChaptersData(response.data);
// //     const response = await studentAPI.getSubjectDetails(subject.id);
// // // getSubjectDetails returns { subject, class, chapters }
// // // but chapters don't have tests array — so we need to fetch tests per chapter
...
// //                   No subjects have been assigned to your class yet. Contact your admin.
// //                 </p>
// //               </div>
// //             ) : (
// //               <div className="space-y-3">
...
// //                     key={subject.id}
// //                     onClick={() => handleSubjectClick(subject)}
// //                     className="w-full bg-white dark:bg-gray-800 rounded-xl border-2 border-gray-200 dark:border-gray-700 hover:border-blue-500 dark:hover:border-blue-500 hover:shadow-md p-5 transition-all flex items-center justify-between group"
// //                   >
// //                     <div className="flex items-center gap-4">
// //                       <div className="w-12 h-12 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center flex-shrink-0">
// //                         <HiBookOpen className="w-6 h-6 text-blue-600 dark:text-blue-400" />
...
// //                           <span>•</span>
// //                           <span>{subject.test_count || subject.total_tests || 0} tests</span>
// //                           {(subject.attempt_count || subject.attempted_tests) > 0 && (
// //                             <>
// //                               <span>•</span>
...
// //   const pendingTests = chapter.tests?.filter(t => !t.attempted) || [];
// //   const attemptedTests = chapter.tests?.filter(t => t.attempted) || [];
// //   const totalTests = chapter.tests?.length || chapter.total_tests || 0;
...
// //         className="w-full p-5 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors"
// //       >
// //         <div className="flex items-center gap-4">
// //           <div className="w-10 h-10 rounded-full bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center flex-shrink-0">
// //             <span className="text-purple-600 dark:text-purple-400 font-bold text-sm">
...
// //                       <span>{test.marks} marks</span>
// //                       <span>•</span>
// //                       <span className="uppercase text-xs font-medium text-blue-600 dark:text-blue-400">
// //                         {test.type}
// //                       </span>
// //                       {test.attempted && test.percentage !== null && (
// //                         <>
...
// //                         <Link
// //                           to={`/student/chapter/${chapter.id}/tests`}
// //                           className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-blue-600 dark:text-blue-400 border border-blue-300 dark:border-blue-600 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors"
// //                         >
// //                           <HiEye className="w-4 h-4" /> View
// //                         </Link>
// //                       </>
...
// //                         onClick={() => onStartTest(test.id)}
// //                         disabled={startingTest === test.id}
// //                         className="flex items-center gap-1 px-4 py-1.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
// //                       >
// //                         {startingTest === test.id ? (
// //                           <span>Starting...</span>
// //                         ) : (
...
// // //       console.log('✅ My tests data:', response.data);
...
// // //       case 'in_progress':
// // //         return <HiClock className="text-orange-500 text-2xl" />;
// // //       case 'failed':
// // //         return <HiXCircle className="text-red-500 text-2xl" />;
// // //       default:
// // //         return <HiClock className="text-gray-500 text-2xl" />;
// // //     }
// // //   };
...
// // //                 <HiAcademicCap className="mr-3 text-blue-600" />
// // //                 My Tests
// // //               </h1>
...
// // //             <HiAcademicCap className="mx-auto text-gray-400 text-6xl mb-4" />
...
// // //                         <span>
// // //                           Date:{' '}
// // //                           {new Date(test.attemptedAt || Date.now()).toLocaleDateString()}
// // //                         </span>
// // //                       </div>
...
// // //                         )}`}
// // //                       >
// // //                         {test.scoreObtained || 0}/{test.totalMarks || 0}
// // //                       </div>
...
// // //                       <button
// // //                         onClick={() =>
// // //                           navigate(`/student/test-attempt/${test._id}/result`)
// // //                         }
...
// // //                     <div className="ml-4">
// // //                       <button
// // //                         onClick={() => navigate(`/student/test/${test.testId}/take`)}
